...
SEAL_DIR = os.environ.get("LIFESAT_SEAL_DIR", str(Path(__file__).resolve().parents[1] / "specs" / "seal"))
        ...
        if got != SEAL_PIN:
            ...
            if claimed != actual:
                problems.append("seal claims contract %s but the pinned contract "
                                "is %s" % (claimed, actual))
            if not (seal.get("accepted_by_user") and seal.get("independently_verified")):
                problems.append("seal is not both user-accepted and independently verified")
