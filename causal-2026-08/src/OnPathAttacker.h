...
class OnPathAttacker : public cSimpleModule, public cListener
{
  protected:
    ...
    Telemetry *capturedFrame = nullptr;   // SP-6 attack arm: byte-identical copy
    int causalFrameIndex = -1;            // frame index within the target contact
    bool hostileConfigSent = false;
    ...
    //
    // With causal mode on, the old A1..A8 paths never run: no rate gate, no three-way
    // random pick, no episode draw. Which intervention happens comes from the scenario,
    // and when it happens comes from the target schedule.

    /** A single, deterministic downlink intervention on the target observation. */
    virtual void causalDownlink(Telemetry *tm, bool& consumed);

    /** SP-2/SP-3 attack arm: uplinks the adversarial configuration command. */
    virtual void injectHostileConfig();

    /** SP-6 attack arm: replays a CAPTURED TELEMETRY frame byte for byte. */
    virtual void replayCapturedTelemetry();
