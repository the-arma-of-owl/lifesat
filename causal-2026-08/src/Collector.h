...
class Collector : public cSimpleModule
{
  ...
  public:
    ...
    /**
     * Records a truth row whose SEMANTIC time is not the writing instant.
     *
     * An episode boundary is a property of the schedule: the window opens when
     * the contact opens, and the writer only notices it at the first telemetry
     * inside that contact.  Stamping the row with the writing instant would
     * make the recorded window start later than the window actually is, and
     * every observation between the two would fall outside it -- which is
     * exactly how the SP-5 pair lost its first observation and the SP-6 pair
     * lost the tm.send rows its duplicate test depends on.
     */
    virtual void recordGroundTruthAt(const Fields& fields, simtime_t at);
