...
class Twin : public cSimpleModule
{
  public:
    /**
     * Writes one d3.alarm row PER channel.
     *
     * The logical row carries the id of the accepted command that wrote the state,
     * or an EXPLICIT null when there is no command.  Absence and null are not the
     * same thing, and the accepted fault signature tests the latter.
     */
    virtual void emitAlarm(const Telemetry *tm, const char *channel,
                           double deviation, double bound, simtime_t dt,
                           bool withCommandLink, bool modeExplained = true,
                           bool digestExplained = true);

    /** True when the ground itself approved this exact command. */
    virtual bool wasApproved(long commandId, long sequence) const;
  ...
  protected:
    ...
    struct ApprovedCommand {
        ...
        double paramValue;
        int targetMode;
        // the id is recorded too: binding a logical alarm to a command means asking
        // whether the ground approved it, which needs the approved ids.
        long commandId = -1;
        long sequence = -1;
    };
