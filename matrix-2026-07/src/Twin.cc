...
void Twin::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        // twin rates differ from the satellite by rateBias; the twin knows only the
        // rateUncertainty bound, not the actual offset.
...
void Twin::stepModel()
{
    // runs regardless of visibility: that is what the intermittent regime means, the
    // ground side predicts across the gap.
...
{
    // measurement term: realigning to the observed value makes the next comparison
    // carry two independent noise draws, so the bound scales by sqrt(2). variance
    // accounting, not threshold tuning.
    double effectiveSigma = resyncOnMatch ? noiseSigma * M_SQRT2 : noiseSigma;
    double measurementTerm = sigmaFactor * effectiveSigma;

    // model term: rate error accumulates over dt, so the bound must widen with the
    // contact gap.
...
{
    if (!enabled) {
        // no twin means no validation gate (the A6s control arm); reaching here is a
    ...
    // the candidate is applied to a copy of the twin's own model: a failed validation
    // must not corrupt the state estimate.
    PowerModel candidate = model;
    if (key == "dischargeRate")
        candidate.setDischargeRate(value);
    else {
        // parameter with no declared physical effect: the gate cannot decide. it fails
        // closed and records a separate verdict rather than uplinking an unevaluated
        // update.
        reason = "parameter has no declared physical effect; outside this envelope";
        updatesUnsupported++;
        // twin-prefixed: this is the twin's internal verdict; the ground station writes
        // the final one (update.unsupported). sharing a name would double count a single
        // semantic event.
        if (collector)
    ...
    // illuminated fraction. the question is the energy balance over the horizon, not the
...
{
    // half-open [from, to): confirmedPrefix never moves backwards, so a command cannot
    // be applied twice.
...
{
    // released commands are recorded with their send time; delayed telemetry cannot be
...
void Twin::observeTelemetry(const Telemetry *tm)
{
    ...
    // temporal bound
    // age of the telemetry at source time, i.e. how stale the twin's picture may be. dt
    // is read here; it is not an alarm criterion.
    ...
    // logical channel
    // telemetry reports the state at sourceTime. which commands had landed by then is
    // generally uncertain: some may have been sent but not arrived. the twin expands
    // the expected state for every admissible prefix and alarms only if the observation
    // matches none. treating uncertainty as an alarm is the error section 3.2 rejects.
    size_t possible = sentByTime(tm->getSourceTime());
    bool logicalBreach = true;
    size_t matched = confirmedPrefix;
    for (size_t n = confirmedPrefix; n <= possible && logicalBreach; n++)
        if (modeAfter(n) == (SatMode)tm->getMode()
            && paramDigestAfter(n) == std::string(tm->getParamDigest())) {
            logicalBreach = false;
            matched = n;
        }
    // on a match the twin pins to that state; on a mismatch it does not advance, so an
    // attacker cannot walk it forward step by step.
    //
    // physical effects of newly confirmed commands are applied when the prefix advances,
    // not at send time: a sent command may never have arrived.
    if (!logicalBreach) {
    ...
    // security channel
    // the satellite's rejection count must match the ground's own. the ground sends
    // only valid commands, so the expected count is constant; an increase is evidence
    // that D1 rejected something the ground never sent. A7c falsifies this counter.
    //
    // alarm on the transition, not the level: re-alarming on every telemetry would
    // turn one event into hundreds of false alarms. the twin adopts the new baseline
    // afterwards, the same estimator logic as confirmedPrefix.
    ...
    if (physicalBreach || logicalBreach || securityBreach) {
        ...
        if (collector)
            collector->logEvent("d3.alarm",
                {{"channel", channel == 0 ? "physical" : (channel == 1 ? "logical" : "security")},
        ...
        // realign to the observed value inside the bound but not outside, or an attacker
        // could drag the twin along step by step.
