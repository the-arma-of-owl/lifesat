...
int TelemetryDescriptor::findField(const char *fieldName) const
{
    ...
    if (strcmp(fieldName, "rejectedCmdCount") == 0) return baseIndex + 6;
    if (strcmp(fieldName, "acceptedCmdCount") == 0) return baseIndex + 7;
    if (strcmp(fieldName, "integrityTag") == 0) return baseIndex + 8;
    return base ? base->findField(fieldName) : -1;
}
